...
## 工具调用格式
要使用工具，请按如下 JSON 代码块格式回复：
```tool
{{"tool": "tool_name", "parameters": {{"param1": "value1"}}}}
```

你可以在一次回复中调用多个工具。我会把执行结果返回给你。

当你发现漏洞时，请使用 report_vulnerability 并提供完整证据。

分析完成后，请按如下格式回复：
```done
{{"status": "complete", "summary": "your analysis summary"}}
```
"""
...
现在开始你的自主分析。请先使用工具探索项目结构，然后系统地寻找潜在漏洞。"""
    ...
    def _parse_done_signal(self, response: str) -> Optional[Dict[str, Any]]:
        """Check for completion signal"""
        done_pattern = r"```done\s*\n?(.*?)\n?```"
        match = re.search(done_pattern, response, re.DOTALL)
        
        if match:
            try:
                return json.loads(match.group(1).strip())
            except json.JSONDecodeError:
                return {"status": "complete", "summary": "Analysis complete"}
    ...
    def _execute_tools(self, tool_calls: List[Dict[str, Any]]) -> str:
        ...
        for call in tool_calls:
            ...
            if self.verbose:
                print(f"  [TOOL] {tool_name}: {json.dumps(parameters)[:80]}...")
    ...
    def run(self) -> Dict[str, Any]:
        """Run the agentic analysis using customized LLM client"""
        if self.verbose:
            print(f"[INFO] Starting agentic vulnerability analysis...")
        ...
        for iteration in range(self.max_iterations):
            ...
            done_signal = self._parse_done_signal(response)
            if done_signal:
                if self.verbose:
                    print(f"\n[COMPLETE] {done_signal.get('summary', 'Analysis complete')}")
                break
            ...
            if not tool_calls:
                self.conversation_history.append({
                    "role": "user",
                    "content": "Please use tools to analyze the code or use report_vulnerability to document findings. When finished, use the done signal."
                })
            else:
